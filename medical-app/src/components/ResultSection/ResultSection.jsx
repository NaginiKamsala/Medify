...
import { ReactComponent as TrueMark } from "../../assets/true_icon.svg";
...
const BookingCard = ({ bookingData, setBookingData }) => {
  ...
  const maxDates = 3;
  let start = useRef(0);
  const [currentDateDisplayList, setCurrentDateDisplayList] = useState(
    dateList.slice(0, 3)
  );
  ...
  function handleNext() {
    if (start.current < 4) {
      start.current++;
      setCurrentDateDisplayList(
        dateList.slice(start.current, start.current + 3)
      );
    } else {
      return;
    }
...
const ResultCard = ({
  ...
  function handleBooking() {
    ...
    enqueueSnackbar(
      `Booking done for ${bookingData.date.getDate()}/${
        bookingData.date.getMonth() + 1
      }/
            ${bookingData.date.getFullYear()}`,
      { variant: "success" }
    );

    setBookingsList([...bookingsList, bookingData]);
  ...
  if (pathname === "/bookings") {
    return (
      <div className={styles.section_wrapper}>
        <div className={styles.top_blue_container}>My Bookings</div>
    ...
    <div className={styles.section_wrapper}>
      ...
      <div className={styles.content_wrapper}>
        <h3 className={styles.heading}>
          {resultsList.length} medical centers available in {cityName}
        </h3>
